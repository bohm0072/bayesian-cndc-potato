...
brms_fit <- function(data,model,Owner,Location,Variety){
  ...
  if (model=="model-1"){
    ...
    # get_prior(formula, family = gaussian, data = d)
    priors <- c(set_prior("normal(3,0.3)", class = "sd", nlpar = "Si", group = "variety:date"), #student_t(3,3,0.3)
                set_prior("normal(3,0.3)", class = "sd", nlpar = "Bmax", group = "variety:date"), #student_t(3,3,0.3)
