...
brms_fit <- function(data,model,Owner,Location,Variety){
  ...
  if (model=="model-1"){
    ...
    # get_prior(formula, family = gaussian, data = d)
    priors <- c(set_prior("student_t(3,3,0.3)", class = "sd", nlpar = "Si", group = "variety:date"), #student_t(3,3,0.1)
                set_prior("student_t(3,3,0.3)", class = "sd", nlpar = "Bmax", group = "variety:date"), #student_t(3,3,0.1)
                ...
                set_prior("normal(4.50,0.20)", nlpar = "alpha1", lb = 0), #"normal(4.50,0.10)"
                set_prior("normal(0.50,0.02)", nlpar = "alpha2", lb = 0, ub = 1), #"normal(0.50,0.01)"
